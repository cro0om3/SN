"""
Snow Liwa - Streamlit App (Fhdd Version)
تطبيق سنو ليوا لحجز التذاكر - نسخة كاملة مستقلة
"""
...
# Load the original HTML file from Fhdd folder
html_file_path = Path(__file__).parent / "1.html"
...
if st.session_state.page == "landing" and not st.session_state.show_booking_form:
    ...
    if html_file_path.exists():
        ...
        # Add JavaScript to communicate with Streamlit
        booking_js = """
        <script>
        function handleBooking(event) {
            ...
            // Trigger Streamlit to show booking form
        ...
        // Scroll to booking section when clicking "احجز تذكرتك الآن"
        ...
        # Hidden button to trigger booking form (fallback)
        if st.button("", key="trigger_booking", help="Show booking form"):
            st.session_state.show_booking_form = True
            st.rerun()
            
    else:
        st.error(f"❌ HTML file not found: {html_file_path}")
        st.info("Showing booking form instead...")
        st.session_state.show_booking_form = True
